...
export default function AgentsPage() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const agents = data.filter((u: any) => u.role === 'AGENT');
      setUsers(agents);
    } catch (error) {
  ...
  const handleDelete = async (userId: string) => {
    if (window.confirm('Are you sure you want to disable/delete this agent?')) {
      try {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Manage agents and their access</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
        <DataTable columns={columns} data={users} searchPlaceholder="Search agents..." />
      </div>

      <Modal
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        title={selectedUser ? 'Edit Agent' : 'Create Agent'}
      >
        <UserForm 
          initialData={selectedUser ? { ...selectedUser } : { role: 'AGENT' }} 
